 ...
 * \author Gustavo Colque y Carlos Urquizo.
 */
...
// #define MOTOR_Y_IN1 16
// #define MOTOR_Y_IN2 17
// #define MOTOR_Z_IN1 5
// #define MOTOR_Z_IN2 18
...
#define FREQ_PWM 200000 // Aumentar la frecuencia del PWM a 20 kHz
#define RES_PWM 8
...
// Función para leer el peso del sensor HX711
float readWeight() {
    float weight = scale.get_units(1); // Promedio de 10 lecturas
    return weight;
}
...
void setup() {
    ...
    scale.set_scale(42884.f/104.7); // Factor de calibración del sensor (ajustar según tu sensor)
...
void loop() {
    ...
    // Leer el peso del sensor HX711
    weight = readWeight();

    // Mostrar el peso en el display LCD
    lcd.setCursor(6, 0);
    ...
    // Mostrar el peso y la velocidad porcentual en la consola serial
    Serial.print("Peso: ");
